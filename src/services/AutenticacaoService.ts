...
const JWT_DEFAULT_EXPIRATION_SECONDS = 24 * 60 * 60;

const converterStringTempoEmSegundos = (valor: string): number | null => {
  const tempo = valor.trim().toLowerCase();
...
const obterExpiracaoJwtEmSegundos = (): number => {
  const valorBruto = process.env.JWT_EXPIRES_IN?.trim();

  if (!valorBruto) {
    return JWT_DEFAULT_EXPIRATION_SECONDS;
  }

  if (/^\d+$/.test(valorBruto)) {
    const horas = Number(valorBruto);
  ...
  const emSegundos = converterStringTempoEmSegundos(valorBruto);
  if (emSegundos && emSegundos > 0) {
    return emSegundos;
  }
...
export class AutenticacaoService {
  async criarUsuario(dados: CriarUsuarioDTO) {
    ...
    // Normaliza tipo_usuario para uppercase
    const tipoUsuarioNormalizado = dados.tipo_usuario.toUpperCase() as
      | "COORDENADOR"
      | "ALUNO"
      | "PACIENTE";
    const tiposValidos = ["COORDENADOR", "ALUNO", "PACIENTE"];

    if (!tiposValidos.includes(tipoUsuarioNormalizado)) {
      throw new Error(
        "tipo_usuario inválido. Valores aceitos: COORDENADOR, ALUNO, PACIENTE",
      );
    }
  ...
  async login(dados: LoginDTO) {
    ...
    const expiracaoJwtEmSegundos = obterExpiracaoJwtEmSegundos();
    const jwtOptions: jwt.SignOptions = {
      expiresIn: expiracaoJwtEmSegundos,
    };
    ...
    const expiraEm = new Date(Date.now() + expiracaoJwtEmSegundos * 1000);
  ...
  async solicitarRecuperacaoSenha(email: string) {
    ...
    dataExpiracao.setHours(
      dataExpiracao.getHours() +
        parseInt(process.env.RESET_TOKEN_EXPIRES_IN || "1"),
    );

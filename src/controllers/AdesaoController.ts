...
export class AdesaoController {
  ...
  async listarPorPaciente(req: AuthRequest, res: Response) {
    try {
      ...
      const resultado = await adesaoService.listarPorPaciente(parseInt(id_paciente), skip, take);
  ...
  async obterEstatisticas(req: AuthRequest, res: Response) {
    try {
      const id_paciente = req.query.id_paciente ? parseInt(req.query.id_paciente as string) : undefined;

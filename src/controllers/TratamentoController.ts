...
export class TratamentoController {
  async criar(req: AuthRequest, res: Response) {
    try {
      ...
      if (!id_paciente || !id_medicamento || !id_farmaceutico || !data_inicio || !frequencia) {
        return res.status(400).json({
          erro: 'id_paciente, id_medicamento, id_farmaceutico, data_inicio e frequencia são obrigatórios',
        });
      }
